...
export default function UserDetail() {
  ...
  useEffect(() => {
    ...
    usersApi
      .get(id)
      .then((data) => { if (!canceled) setUser(data) })
      .catch((e) => { if (!canceled) setError(e.message || 'Failed to load user') })
  ...
  const statusClass =
    user.status === 'active' ? 'badge-success-vibrant'
    : user.status === 'inactive' ? 'badge-danger-vibrant'
    : 'badge-muted-vibrant'
  ...
  return (
    <div className="stack gap-32 user-profile-container">
        ...
        <div className="hero-main-row">
          ...
          <div className="hero-side-stack">
            <div className="hero-stat-card">
              <span className="hero-stat-label">Access role</span>
              <span className="hero-stat-value">{user.role || '—'}</span>
            </div>
            <div className="hero-stat-card">
          ...
          <section className="detail-card accent-card">
            ...
            <div className="detail-card-body">
              ...
              <Link to={`/users/${user.id}/edit`} className="converted-link-premium">
                <div className="link-icon"><Icon name="edit" /></div>
      ...
      <style>{`
        ...
        .detail-card-badge.subtle {
          color: var(--text-dimmed);
        }

        .badge-success-vibrant { background: rgba(16, 185, 129, 0.15); color: #ffffff; border: 1px solid rgba(16, 185, 129, 0.3); }
        .badge-danger-vibrant  { background: rgba(239, 68, 68, 0.15);  color: #ffffff; border: 1px solid rgba(239, 68, 68, 0.3);  }
        .badge-muted-vibrant   { background: rgba(148, 163, 184, 0.12); color: #ffffff; border: 1px solid rgba(148, 163, 184, 0.2); }
