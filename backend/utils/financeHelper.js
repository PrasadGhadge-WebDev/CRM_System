...
async function autoGenerateInvoice(deal, userId) {
  try {
    ...
    const invoice = await Invoice.create({
      ...
      items: [{ description: `Product/Service for Deal: ${deal.name}`, quantity: 1, price: deal.value, amount: deal.value }],
      subtotal: deal.value,
      ...
      company_info: { name: company?.name || 'Company Name', address: company?.address || '', phone: company?.phone || '', email: company?.email || '' },
      customer_info: { name: customer?.name || 'Customer Name', phone: customer?.phone || '', email: customer?.email || '', address: customer?.address || '' },
      created_by: userId
    });
...
/**
 * Automatically creates a PENDING payment record for a WON deal/invoice.
 */
async function autoGeneratePendingPayment(deal, invoice, userId) {
  try {
    const paymentNumber = await getNextPaymentNumber(deal.company_id);
    ...
    const payment = await Payment.create({
      ...
      notes: `Expected payment for Deal: ${deal.name}`,
      created_by: userId
    });

    return payment;
  } catch (error) {
    console.error('Error auto-generating pending payment:', error);
    return null;
  }
}

module.exports = { autoGenerateInvoice, autoGeneratePendingPayment };
