...
exports.createUser = asyncHandler(async (req, res) => {
  ...
  if (user.status === 'active') {
    notifier.sendUserCreationEmail(
      user.email,
      user.name,
      password,
      user.role,
...
exports.getUser = asyncHandler(async (req, res) => {
  ...
  const user = await User.findOne(filter).populate('company_id', 'company_name status');
  if (!user) {
...
exports.updateUser = asyncHandler(async (req, res) => {
  ...
  const filter = { _id: req.params.id };
  if (req.user.role !== 'Admin') filter.company_id = req.user.company_id;
  if (req.user.role === 'Manager') filter.manager_id = req.user.id || req.user._id;
...
exports.deleteUser = asyncHandler(async (req, res) => {
  const dFilter = { _id: req.params.id };
  if (req.user.role !== 'Admin') dFilter.company_id = req.user.company_id;
  if (req.user.role === 'Manager') dFilter.manager_id = req.user.id || req.user._id;

  const user = await User.findOne(dFilter).select('+password');
  if (!user) {
    return res.fail('User not found', 404);
  }
  if (isHRUser(req) && user.role !== 'Employee') {
...
exports.resetUserPassword = asyncHandler(async (req, res) => {
  ...
  const pFilter = { _id: req.params.id };
  if (req.user.role !== 'Admin') pFilter.company_id = req.user.company_id;
  if (req.user.role === 'Manager') pFilter.manager_id = req.user.id || req.user._id;

  const user = await User.findOne(pFilter).select('+password');
  if (!user) {
    return res.fail('User not found', 404);
  }
  if (isHRUser(req) && user.role !== 'Employee') {
